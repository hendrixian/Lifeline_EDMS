...
<%
    ...
    String queryQ = "SELECT LocationUsername, Amount, Date " +
            "FROM contributions c1 " +
            "WHERE ShelterName = (SELECT sheltername FROM representative WHERE Representativename = '"+loggedInRepresentative+"') " +
            "AND Amount = (SELECT MAX(Amount) FROM contributions c2 WHERE c1.LocationUsername = c2.LocationUsername) " +
    ...
    try {
        ...
        while (rsQ.next()) {
            String[] row = {
                    rsQ.getString("LocationUsername"),
        ...
        <div class="my-container mb-6 w-full">
            <div class="shadow-lg rounded-lg p-5 grow w-full">
                <div class="flex flex-wrap gap-5 justify-center w-full">
                    ...
                    %>
                    <a href="AddContribution.jsp?LocationUsernameFromLocationProfile=<%=LocationUsername%>" class="flex-1 min-w-[150px] max-w-[250px]">
                    ...
                    %>
                    <a href="AddPost.jsp?LocationUsernameFromLocationProfile=<%=LocationUsername%>" class="flex-1 min-w-[150px] max-w-[250px]">
                        <button class="btn btn-warning hover:bg-amber-500 w-full text-center ">Add Post</button>
                    </a>
                    <%
                        }
                    %>
